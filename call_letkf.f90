SUBROUTINE CALL_LETKF_2D(nx,ny,nbv,num_local_obs,msw_infl,eps,R,Correlation_Par,&
    ...
    Bias_Model_Dim,Bias_Obs_Dim,Bias_Model_Uniform_STD,&
    Bias_Obs_Uniform_STD,Model_Inflation_Uniform_STD,&
    minimize_lbfgsb_n,minimize_lbfgsb_m,minimize_lbfgsb_iprint,&
    minimize_lbfgsb_epsilon_in,minimize_lbfgsb_factr,minimize_lbfgsb_pgtol)
    ...
    INTEGER,INTENT(IN) :: h(ny,nx),Parameter_Optimization_Flag,Par_Sens_Dim,State_DIM_Single_Layer
    INTEGER,INTENT(IN) :: State_DIM_Single_Column,Def_Localization,Normal_Score_Trans
    ...
    INTEGER :: iwt, ierror
    REAL(8) :: wt(nbv), XF_NNscore_Sort(nbv), XF_Trans_Sort(nbv), HXF_NNscore_Sort(nbv), HXF_Trans_Sort(nbv)
    REAL(8) :: Observation_NNscore_Sort(1), Observation_Trans_Sort(1), HXF_Trans_Var, HXF_NNscore_Var
    ...
    !$OMP PARALLEL SHARED(xa,parm_infl,dxf,h4d,hxf,hxfm,h,hdxf,increments,localization_map,&
    ny_loc,dxf_dev,dxa_dev,dxa) PRIVATE(ix,xa_median,parm,ii,jj,num_local_obs_temp,dist,coeffs,&
    dist_pack,ny_pack,coeffs_pack,rdiag_pack,rloc_pack,d_pack,hdxf_pack,coeffs_whole,coeffs_whole_eps,&
    coeffs_whole_copy,coeffs_whole_max_value,coeffs_whole_init,coeffs_whole_index,i,localobs,j,it,&
    localobs_index,trans,xa_xm,dxa_min,dxa_max,dxa_mean,seed,U,xa_median_ens, Analysis_NNscore, Analysis_Trans, Par_STD_Index)
    !$OMP DO SCHEDULE(STATIC)
    ...
    DO ix=1,nx
        ...
        IF (ny_pack > 0) THEN
            IF (Def_Print >= 3) THEN
                PRINT*,"Correlation_Par",Correlation_Par(4,1), vario_type, Correlation_Par(5,1),&
                "GridSize_Sys",GridSize_Sys,"ny_pack",ny_pack,"dist_pack",minval(dist_pack),maxval(dist_pack)
            END IF
            ...
            IF (ny_loc(ix) > 0) THEN
                ...
                IF((Parameter_Optimization_Flag>0) .AND. (Alpha_Inflation .NE. 0.0) .AND. (Par_Sens_Dim >= 1) .AND. &
                (ix>State_DIM_Single_Layer)) THEN
